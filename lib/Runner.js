...
async function runTool({nodeCommandLine, toolName, ondemand, callback}) {
  ...
  waitForWSEndpoint(nodeProcess, webSocket => runToolForProcess({webSocket, toolName, ondemand, callback}));
...
async function runToolForProcess({webSocket, toolName, ondemand, callback}) {
  const connection = new Connection(new WebSocket(webSocket));
  const tool = createTool(toolName, {connection});
  tool.on('reportStart', callback.bind(null, 'reportStart'));
  tool.on('reportChunk', callback.bind(null, 'reportChunk'));
  tool.on('reportFinish', callback.bind(null, 'reportFinish'));
